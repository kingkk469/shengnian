...
- Windows 和苹果芯片 Mac 均通过自动化回归；Mac 独立应用已通过实际转写、声纹和原生界面检查。
- Mac 版面向苹果芯片、macOS 14+；默认 CPU 转写，不承诺 MPS 加速，物理麦克风、热插拔、睡眠恢复和长时间录音仍待硬件验收。
- 从源码运行时，第一次下载 FunASR 模型可能较慢；Mac 独立应用已内置模型。
